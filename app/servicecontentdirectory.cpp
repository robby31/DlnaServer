...
ServiceContentDirectory::ServiceContentDirectory(UpnpControlPoint *upnp,  QNetworkAccessManager *nam, QString host, int port, QObject *parent):
    QObject(parent),
    netManager(nam),
    m_upnp(upnp),
...
//            }
    ...
    else if (request->operationString() == "SUBSCRIBE" && request->url().toString() == "/upnp/event/content_directory" && request->header("NT") == "upnp:event")
    {
        QStringList l_url;
    ...
    else if (request->operationString() == "SUBSCRIBE" && request->url().toString() == "/upnp/event/content_directory" && !request->header("SID").isEmpty())
    {
...
void ServiceContentDirectory::sendEvent(const QString &uuid)
{
...
void ServiceContentDirectory::sendEventReply()
{
